...
export class DocumentController {
  ...
  @ApiOperation({ summary: 'Presign a document for upload' })
  @ApiResponse({ status: 200, description: 'Document presigned successfully.' })
  @ApiResponse({ status: 401, description: 'JWT signature pass missing or invalid.' })
  async presignDocument(@Body() body: PresignDocumentDto, @Req() req: any) {
    return this.documentService.presignDocument(body, req.user.id as string);
  }

  @Post('complete/:id')
  @ApiBearerAuth()
  @ApiOperation({ summary: 'Complete a document upload by its ID' })
  @ApiResponse({ status: 200, description: 'Document upload completed successfully.' })
  @ApiResponse({ status: 401, description: 'JWT signature pass missing or invalid.' })
  @ApiResponse({ status: 404, description: 'Document not found.' })
  @ApiResponse({ status: 400, description: 'Document is not pending.' })
  async completeDocumentUpload(@Param('id') id: string, @Req() req: any) {
    return this.documentService.completeDocument(id, req.user.id as string);
  }
